...
class Middleend(fsmHandle: ChiselFSMHandle, p: CacheParams, address: UInt) {
    ...
    def read(buffer: Vec[UInt], nextAddress: UInt, tag: UInt, offset: UInt, hit: Bool, 
      readDone: Bool, cpu: CacheIO, mainMem: NastiBundle) = {
      ...
      val wdata = Cat(mainMem.r.bits.data, Cat(buffer.init.reverse))
      ...
      when(readDone) {
          ...
          val writeVec = VecInit.tabulate(p.nWords * p.wBytes)(i => wdata.asUInt((i + 1) * p.byteBits - 1, i * p.byteBits))
          localMemory.write(index, writeVec)
      }

      //Take the cache line and split it into its words, get the word we want
      val readPort = localMemory.read(nextIndex, !readDone) //get started on the next PC, will take 1 clock cycle to get the data
